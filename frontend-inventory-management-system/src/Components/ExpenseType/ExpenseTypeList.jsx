...
import {
  ...
  ExpenseTypeListRequestAPI,
  ExpenseTypeDeleteRequestAPI,
...
const ExpenseTypeList = () => {
  ...
  useEffect(() => {
    ExpenseTypeListRequestAPI(1, perPage?.value, searchKeyword);
  }, []);
  ...
  const perPageOnChange = async (e) => {
    ...
    await ExpenseTypeListRequestAPI(1, e.value, searchKeyword);
  };
  ...
  const searchKeywordOnChange = async (e) => {
    ...
    if (e.target.value.length === 0) {
      setSearchKeyword("0");
      await ExpenseTypeListRequestAPI(1, perPage?.value, "0");
    } else {
      await ExpenseTypeListRequestAPI(1, perPage?.value, e.target.value);
    }
  };

  const deleteExpenseTypeList = async (id) => {
    let text =
      "Do you really want to delete this account? This process cannot be undone!";
    let deleteRequestAPI = ExpenseTypeDeleteRequestAPI;
    let refreshAPI = ExpenseTypeListRequestAPI;
  ...
  const showSingleExpenseTypeList = async (id) => {
    handelClosePopup();
    await ExpenseTypeDetailsByID(id);
  };

  const singleExpenseTypeListData = useSelector(
  ...
  return (
    <div className='bg-white relative'>
      <div className='container mx-auto bg-white  pt-5'>
                        ...
                        <tr key={index}>
                          ...
                          <td className='px-4 py-3 border-b border-gray-200 bg-white text-xs'>
                            <div className='flex gap-2'>
                              <Link to={`/ExpenseTypeList/update/${item?._id}`}>
                                {" "}
                              ...
                              <span
                                ...
                                onClick={() =>
                                  showSingleExpenseTypeList(item?._id)
                                }
                              >
                                <button
                                  aria-hidden
                              ...
                              <button
                                ...
                                onClick={() => deleteExpenseTypeList(item?._id)}
                              >
                                <span
                                  aria-hidden
        ...
        {/* Single ExpenseTypeList View Popup */}
              ...
              <div className='md:w-[700px] md:h-[300px] bg-[#fff] rounded-lg relative flex  items-center px-5'>
                <div className='relative md:flex  bg-white w-full gap-4'>
                  <div className='w-full mt-4 md:mt-0 md:w-1/3 bg-white grid place-items-center overflow-hidden'>
                    <img
                      src={singleExpenseTypeListData?.Image}
                      alt='tailwind logo'
                  ...
                  <div className='w-full md:w-2/3  bg-white flex flex-col space-y-2 p-3'>
                    <div className='flex justify-between item-center'>
                      <p className='text-gray-500 font-medium hidden md:block'>
                        Single Expense Type List Data
                      </p>
                      <div>
                        <div className=' px-3 py-[1px] mr-5 rounded-full text-xs font-medium bg-amber-500 text-slate-700 hidden md:block'>
                          Create Date:{" "}
                          <Moment format='DD/MM/YYYY'>
                            {singleExpenseTypeListData?.CreatedDate}
                          </Moment>
                        </div>
                      </div>
                    </div>
                    <h3 className='font-black text-gray-800 md:text-xl text-xl'>
                      {singleExpenseTypeListData?.ExpenseTypeListName}
                    </h3>
                    <ul>
                      <li>Expense Type Id: {singleExpenseTypeListData?._id}</li>
                      <li>
                        Expense Type Name:{" "}
                        {singleExpenseTypeListData?.ExpenseName}
                      </li>
