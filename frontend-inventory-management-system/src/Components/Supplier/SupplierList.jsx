...
import {
  SupplierDeleteRequestAPI,
  SupplierDetailsByID,
  SupplierListRequestAPI,
} from "../../API/SupplierAPIRequest";
...
const SupplierList = () => {
  ...
  useEffect(() => {
    SupplierListRequestAPI(1, perPage?.value, searchKeyword);
  }, []);
  ...
  const handelPageClick = async (event) => {
    SupplierListRequestAPI(event.selected + 1, perPage?.value, searchKeyword);
  };

  const perPageOnChange = async (e) => {
    setPerPage({ value: parseInt(e.value), label: e.label });
    await SupplierListRequestAPI(1, e.value, searchKeyword);
  };
  ...
  const searchKeywordOnChange = async (e) => {
    ...
    if (e.target.value.length === 0) {
      setSearchKeyword("0");
      await SupplierListRequestAPI(1, perPage?.value, "0");
    } else {
      await SupplierListRequestAPI(1, perPage?.value, e.target.value);
    }
  };
  ...
  const deleteSupplier = async (id) => {
    ...
    let deleteRequestAPI = SupplierDeleteRequestAPI;
    let refreshAPI = SupplierListRequestAPI;
  ...
  const showSingleSupplier = async (id) => {
    handelClosePopup();
    await SupplierDetailsByID(id);
  };

  const singleSupplierData = useSelector(
    (state) => state.supplier.singleSupplier
  );
                        ...
                        <tr key={index}>
                          ...
                          <td className='px-4 py-3 border-b border-gray-200 bg-white text-xs'>
                            <div className='flex gap-2'>
                              <Link to={`/SupplierList/update/${item?._id}`}>
                                {" "}
                              ...
                              <span
                                ...
                                onClick={() => showSingleSupplier(item?._id)}
                              >
                                <button
                                  aria-hidden
                    ...
                    <div className='flex justify-between item-center'>
                      ...
                      <div>
                        <div className=' px-3 py-[1px] mr-5 rounded-full text-xs font-medium bg-amber-500 text-slate-700 hidden md:block'>
                          Create Date:{" "}
                          <Moment format='DD/MM/YYYY'>
                            {singleSupplierData?.CreatedDate}
                          </Moment>
                        </div>
                      </div>
                    </div>
                    <h3 className='font-black text-gray-800 md:text-xl text-xl'>
                      {singleSupplierData?.SupplierName}
                    </h3>
                    <ul>
                      <li>User Id: {singleSupplierData?._id}</li>
                      <li>Email: {singleSupplierData?.Email}</li>
                      <li>Phone: {singleSupplierData?.Phone}</li>
                      <li>Address: {singleSupplierData?.Address}</li>
                      <li>Type: {"Supplier"}</li>
                    </ul>
                  </div>
                </div>
